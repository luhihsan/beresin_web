...
export default function GuestTicketForm({ onCancel, onRefresh, customers = [] }) {
  ...
  // SOLUSI BUG MEMORY LEAK: Gunakan mutable useRef untuk mengunci kelistrikan kamera
  const streamRef = useRef(null);
  const videoRef = useRef(null);
  
  // STATE PROFIL INTEGRASI
  ...
  // PROTEKSI TOTAL: Matikan perangkat kamera keras secara instan saat unmount komponen
  useEffect(() => {
    return () => {
  ...
  // PROFIL KASIR DETECTOR: Membaca profil terpilih dan menarik seluruh aset mobil mereka dari Cloud
  const handleCustomerProfileSelect = async (phoneId) => {
    ...
    if (matchedCustomer) {
      ...
      // Tarik berkas mobil terikat ke NoSQL secara realtime
      try {
        ...
        console.error("Gagal memuat ekosistem mobil pelanggan terdaftar:", err);
      } finally {
  ...
  // AUTO-POPULATE ENGINE: Mengisi instan 8 kolom spesifikasi mobil dari database
  ...
  const toggleCamera = async (activate) => {
    if (activate) {
      try {
        ...
        alert("Sistem gagal mengakses perangkat kamera fisik.");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // 1. Sinkronisasi CRM: Daftarkan/Perbarui Dokumen Profil Utama Pelanggan
      ...
      // 2. Jika ini mobil baru, daftarkan ke koleksi kendaraan. Jika mobil lama, lewati jalur tulis ini.
      ...
      alert("Berkas pendaftaran antrean dan rekam profil berhasil disimpan.");
      onCancel();
      await onRefresh();
    } catch (err) { alert(`Kegagalan sistem: ${err.message}`); } finally { setIsSubmitting(false); }
  };

  return (
    <form onSubmit={handleSubmit} className="space-y-4 text-left">
      
      {/* DROPDOWN DETEKSI PROFIL LAMA */}
      <div className="bg-slate-950 p-4 rounded-2xl border border-slate-800 space-y-3">
        <div className="space-y-1.5 text-xs">
          <label className="font-semibold text-blue-400">Pilih dari Database Profil Pelanggan Tetap (Opsional)</label>
          <select 
          ...
          >
            <option value="">-- Cari / Pilih Nama Pelanggan Guest --</option>
        ...
        {/* DROPDOWN PILIHAN MOBIL SINKRONISASI */}
        {existingVehicles.length > 0 && (
          <div className="space-y-1.5 text-xs animate-fade-in">
            <label className="font-semibold text-emerald-400">Pilih Unit Kendaraan Terdaftar Milik Pelanggan</label>
            <select 
              value={selectedVehicleId} 
              onChange={(e) => handleVehicleSelect(e.target.value)}
              className="w-full px-3 py-2.5 bg-slate-900 border border-slate-800 rounded-xl text-white outline-none cursor-pointer text-xs"
            >
              <option value="">-- Gunakan Kendaraan Baru (Isi Manual) --</option>
        ...
        {isLoadingVehicles && <p className="text-[11px] text-slate-500 animate-pulse">Menghubungkan ke pangkalan data kendaraan...</p>}
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
        <div className="space-y-1.5">
          <label className="font-semibold text-slate-400">Nama Lengkap Pelanggan</label>
          <input type="text" required value={formData.customerName} onChange={(e) => setFormData({...formData, customerName: e.target.value})} placeholder="Nama Lengkap Pemilik" className="w-full px-3 py-2.5 bg-slate-950 border border-slate-800 rounded-xl text-white outline-none" />
        </div>
        <div className="space-y-1.5">
          <label className="font-semibold text-slate-400">Nomor Telepon / WhatsApp</label>
          <input type="tel" required value={formData.customerPhone} onChange={(e) => setFormData({...formData, customerPhone: e.target.value})} placeholder="Nomor WhatsApp Aktif" className="w-full px-3 py-2.5 bg-slate-950 border border-slate-800 rounded-xl text-white font-mono outline-none" />
        </div>
      </div>
